...
public class CloudTaskApiDelegateImplTest {
    ...
    public void updateUserById() throws Exception {
        ...
        UserDTO userDTO = UserEntityMapper.INSTANCE.convertUserEntityToUserModel(userEntity);
        ResponseEntity<UserDTO> res = ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(userDTO);
        when(userService.updateUserById(userEntity.getId(), new UserUpdateRequest())).thenReturn(res);

        mockMvc.perform(
            put("/api/v2/cloud-task/user/" + userEntity.getId().toString())
                .content("{}")
                .contentType(MediaType.APPLICATION_JSON)
            )
            .andExpect(status().isOk())
